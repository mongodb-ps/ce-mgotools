...
const (
	DateFormatCtimenoms    = DateFormat("Mon Jan _2 15:04:05")
	DateFormatCtime        = DateFormat("Mon Jan _2 15:04:05.000")
...
func IsIso8601String(date string) bool {
	parts := []rune(date)

	// Use a separate statement for the bound check because combining it
	// with the return statement below generates 19 bound checks.
	if len(parts) < 19 {
		return false
	}

	return unicode.IsNumber(parts[0]) &&
